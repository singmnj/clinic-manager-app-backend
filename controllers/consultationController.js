...
const createConsultationForPatient = async (request, response) => {
	...
	if (patientDoc) {
		let newConsultationDoc = await Consultation.create({
			...request.body,
			patientId: patientId,
		});
...
const updateConsultation = async (request, response) => {
	...
	let updatedConsultationDoc = await Consultation.findByIdAndUpdate(
		consultationId,
		request.body
	).exec();
...
const deleteConsultation = async (request, response) => {
	let consultationId = request.params.cid;
	let deletedConsultationDoc =
		await Consultation.findByIdAndDelete(consultationId).exec();
